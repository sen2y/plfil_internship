export function analysisSalesAcc(list) {
    let totalDataAcc;
    ...
    for (let k = 0; k < result.length; k++) {
        ...
        tmpCount = cnt;
        cnt += result[k].amount;
        result[k].amount += tmpCount;
    }
    cnt = 0;
    for (let k = 0; k < result2.length; k++) {
        let tmpCount;
        tmpCount = cnt;
        cnt += result2[k].amount;
        result2[k].amount += tmpCount;
    }
    cnt = 0;
    for (let k = 0; k < result3.length; k++) {
        let tmpCount;
        tmpCount = cnt;
        cnt += result3[k].amount;
        result3[k].amount += tmpCount;
    }
