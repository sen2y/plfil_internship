...
export const MyResponsiveBar7 = ({ data }) => {
    ...
    function countDate(list) {
        ...
        const createYM2 = []; // ex ->  [{Date:21-08-17, count: 12}, {Date:21-09-21, count: 5},,,]
        for (let i = 0; i < list.length; i++) {
            ...
            const createDate = actorProfile.createdAt.substr(2, 8);
            ...
            if (createYear in createYears) {
                ...
                for (let k = 0; k < createYM2.length; k++) {
                    // 배열에 있는 Date
                    if (createDate == createYM2[k].Date) {
                        find = k; break;
                    }
                }
                ...
                if (find !== false) {
                    ...
                    createYM2.push({ Date: createDate, count: 1 }); // 배열 추가
                }
            }
            // 새로운 Date
        ...
        let result = createYM2.sort(function (a, b) { return a.Date > b.Date ? 1 : -1 }); // year 기준 sort
